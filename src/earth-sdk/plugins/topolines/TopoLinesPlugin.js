...
export class TopoLinesPlugin {
    ...
    set cartoColor(v) {
        if (!this._cartoColor.equals(v)) {
            this._cartoColor.copy(v);
            this.updateDefines();
        }
    }
    ...
    constructor(options = {}) {
        const isPlanar =
            "projection" in options ? options.projection === "planar" : true;
        ...
        const {
            ...
            topoOpacity = 0.5,
            topoLimit = isPlanar
                ? new Vector2(0.1, 1e10)
                : new Vector2(1, 1e10),
            topoFadeLimit = isPlanar
                ? new Vector2(0, 1e10)
                : new Vector2(0, 1e4),
            ...
            cartoFadeLimit = isPlanar
                ? new Vector2(0, 1e10)
                : new Vector2(1.5 * 1e4, 1e6),
        } = options;
        ...
        this._cartoColor = new Color().set(cartoColor);
    ...
    computeTopographicLineInfo(camera, worldPos, target = {}) {
        ...
        const distanceFromCamera = _vec
            .copy(worldPos)
            .applyMatrix4(viewMatrix).z;
        const p0 = _p0
            .set(0, 0, distanceFromCamera)
            .applyMatrix4(camera.projectionMatrix);
        const p1 = _p1
            .set(1, 1, distanceFromCamera)
            .applyMatrix4(camera.projectionMatrix);

        // amount of pixel change per meter in screen space
        // multiple by 0.5 since the NDC value range is between [-1, 1]
        const pixelDelta = _vec2
            .subVectors(p1, p0)
            .multiply(resolution)
            .multiplyScalar(0.5);
        ...
        const pixelsPerMeter = Math.max(
            Math.abs(pixelDelta.x),
            Math.abs(pixelDelta.y)
        );
        ...
        const maxFadeLimitAlphaTopo =
            1.0 -
            MathUtils.smoothstep(
                topoFadeLimit.y * (1.0 - FADE_SIZE_HALF),
                topoFadeLimit.y * (1.0 + FADE_SIZE_HALF),
                Math.pow(10.0, nearestPow10 + 1.0)
            );
        const minFadeLimitAlphaTopo = MathUtils.smoothstep(
            cartoFadeLimit.y * (1.0 - FADE_SIZE_HALF),
            cartoFadeLimit.y * (1.0 + FADE_SIZE_HALF),
            Math.pow(10.0, nearestPow10 + 1.0)
        );
    ...
    init(tiles) {
        ...
        this._loadModelCallback = ({ scene }) => {
            scene.traverse((c) => {
                if (c.material) {
                    const params = wrapTopoLineMaterial(
                        c.material,
                    ...
                    params.cartoColor.value = this._cartoColor;
                    ...
                    c.material.defines.USE_TOPO_ELLIPSOID = Number(
                        this.projection === "ellipsoid"
                    );
